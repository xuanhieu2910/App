...
export async function readAndValidateBuildConfigAsync(configPath, params = {}) {
...
export function validateAllFunctionsExist(config, { externalFunctionIds = [], skipNamespacedFunctionsCheck }) {
    ...
    if (nonExistentFunctions.length > 0) {
        throw new BuildConfigError(`Calling non-existent functions: ${nonExistentFunctions.map((f) => `"${f}"`).join(', ')}.`);
    }
}
